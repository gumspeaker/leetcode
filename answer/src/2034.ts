class StockPrice {
  ...
  currentDay: number;
  constructor() {
    this.big = new Heap((a, b) => b[1] - a[1]);
    this.small = new Heap((a, b) => a[1] - b[1]);
    this.map = new Map();
    this.currentDay = 0;
  }

  update(timestamp: number, price: number): void {
    if (timestamp > this.currentDay) {
      this.currentDay = timestamp;
    }

    this.big.add([timestamp, price]);
    this.small.add([timestamp, price]);
    this.map.set(timestamp, price);
  }

  current(): number {
    return this.map.get(this.currentDay)!;
  }

  maximum(): number {
    while (true) {
      const [timestamp, price] = this.big.peek()!;
  ...
  minimum(): number {
    while (true) {
      const [timestamp, price] = this.small.peek()!;
      if (this.map.get(timestamp) == price) {
        return price;
      }
      this.small.poll();
